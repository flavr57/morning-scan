...
# ── Item ID for dismiss tracking ────────────────────────────────────────

def item_id(item):
    """Generate a stable ID for dismiss tracking via localStorage."""
    key = f"{item.get('platform', '')}|{item.get('title', '')}|{item.get('link', '')}"
...
def render_lead_item(item):
    ...
    return f'''<div class="lead-item" data-id="{iid}">
...
def render_scan_item(item):
    ...
    link_html = f'<a class="item-link" href="{link}" target="_blank">View &rarr;</a>'
