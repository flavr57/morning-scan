...
Reads scan-data.json (pushed by the local scanner) and produces index.html.
All display text (meta lines, body summaries, categories) is synthesized
from raw scraper fields: platform, title, pay, duration, link, alert, flagged.
No hand-written content needed.
"""
...
# ── Platform categories (derived automatically) ─────────────────────────
PLATFORM_CATEGORY = {
    "Respondent": "Research",
    "UserInterviews": "Research",
    "Contra": "Freelance",
    "Central Casting": "Acting",
    "Casting Networks": "Acting",
    "Everyset": "Acting",
    "LinkedIn": "Jobs",
    "Gmail": "Email",
    "eBay": "Marketplace",
...
HIGH_SIGNAL_PLATFORMS = {
    ...
    "Casting Networks", "Everyset", "Contra",
}


# ── Derive display fields from raw scraper data ─────────────────────────

def build_meta(item):
    """Build the grey meta line from whatever fields are present."""
    parts = []
...
def build_lead_body(item):
    """Build a short description for above-fold items from available data."""
    parts = []
    if item.get("duration"):
        parts.append(item["duration"])
    if item.get("deadline"):
...
def get_category(item):
    """Get display category from platform or explicit field."""
...
def score_item(item):
    ...
    if pay_str:
        try:
            ...
            # Non-numeric pay like "SAG rate" still gets a small bump
            s += 10
...
def render_scan_item(item):
    ...
    is_gmail = platform == "Gmail"
    is_flagged = item.get("flagged", False)

    # Gmail flagged items get the special tinted card
    if is_gmail and is_flagged:
        link_html = f'<a href="{link}" target="_blank" class="gmail-action">Open in Gmail &rarr;</a>'
        return f'''<div class="gmail-item">
          <p class="gmail-title">{title}</p>
          <p class="gmail-meta">{meta + " &middot; " if meta else ""}{link_html}</p>
        </div>'''

    # Regular Gmail (not flagged) — render as normal item with link
    if is_gmail:
        link_html = f'<a class="item-link" href="{link}" target="_blank">Open in Gmail &rarr;</a>'
    ...
    # All other platforms: show pay pill OR view link (not both)
...
def generate_page(data):
    ...
    # ── Group by platform for Full Scan ──────────────────────────────
    grouped = {}
    for it in all_items:
        if it.get("error"):
            continue
    ...
    # Combine eBay sub-platforms + FB Marketplace under one heading
    ebay_combined = (
        grouped.pop("eBay", [])
    ...
    # Always show the combined section (even if empty)
    grouped["eBay / Marketplace"] = ebay_combined + fb

    # Column assignments
    col1_platforms = ["Central Casting", "Casting Networks", "Everyset"]
    col2_platforms = ["Respondent", "UserInterviews", "Contra"]
    col3_platforms = ["LinkedIn", "Gmail", "eBay / Marketplace"]
